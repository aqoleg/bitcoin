...
public class KeyPairTest extends Test {

    public static void main(String[] args) {
        new KeyPairTest().testAll();
    }

    public void create() {
        assertNotThrows(() -> new KeyPair(true));

        assertThrows(NullPointerException.class, () -> new KeyPair(null, true));
        assertThrows(KeyPair.Exception.class, () -> new KeyPair(BigInteger.valueOf(-1), true));

        byte[] bigKey = new byte[33];
        bigKey[0] = 1;
        assertThrows(KeyPair.Exception.class, () -> new KeyPair(new BigInteger(bigKey), true));

        byte[] bytes = new byte[32];
        bytes[30] = 2;
        bytes[29] = 1;
        KeyPair keyPair = new KeyPair(new BigInteger(1, bytes), false);
        assertEquals("10200", keyPair.privateKey.toString(16));
        assertTrue(!keyPair.publicKey.compressed);
    }

    public void wif() {
        assertThrows(NullPointerException.class, () -> KeyPair.decode(null));
        assertThrows(
                KeyPair.Exception.class,
    ...
    private void hexWifTest(String hex, String wif) {
        KeyPair key = KeyPair.decode(wif);
        assertEquals(hex, key.privateKey.toString(16).toUpperCase());
        assertEquals(wif, (new KeyPair(new BigInteger(hex, 16), key.publicKey.compressed)).encode());
    }

    private void wifToAddress(String wif, String address) {
        assertEquals(address, KeyPair.decode(wif).getAddress());
    }

    private void hexToAddress(String hex, String address, boolean compressed) {
